...
def isvalid(df):
    ...
    # print(df.query("videos == '7.mp4'")["scores"].item() == 5)
    cond4 = not (df.query("videos == '7.mp4'")["scores"].item() == 5 and len(df['scores'].unique()) > 2)
    cond5 = not (df.query("videos == '6.mp4'")["scores"].item() == 5 and len(df['scores'].unique()) > 2)
    cond6 = not (df.query("videos == '5.mp4'")["scores"].item() == 5 and len(df['scores'].unique()) > 2)

    # cond5 = df.query("videos == '4.mp4'")["sanityscore"].item() != 2
    # cond6 = df.query("videos == '3.mp4'")["sanityscore"].item() != 2
    ...
    return cond2 and cond3 and cond4 and cond5 and cond6 # and cond4 and cond5 and cond6
